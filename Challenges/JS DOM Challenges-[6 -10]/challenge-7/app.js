...
accordionButtons.forEach( button => {
  button.addEventListener('click' , function () {
    ...
    const activeAccordionButton = this.parentElement ;

    //set the parent class as active
    activeAccordionButton.classList.toggle('active');

    //remove other class as active 
    accordionButtons.forEach( btn => {
      
      const nextAccordionButton = btn.parentElement ;
      if(nextAccordionButton !== activeAccordionButton) {
        nextAccordionButton.classList.remove('active');
      }
    });
});
});